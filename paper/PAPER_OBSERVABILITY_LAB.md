...
7. **Z3 SMT formal verification** providing machine-checkable proofs of four framework invariants: GCR architectural independence, SS cubic derivation, SS strict monotonicity, and SS boundary conditions. All four theorems verified in 1.66 ms total (Section 8).
8. **Adversarial Red-on-Blue evaluation protocol** resolving supervisor circularity through structured dialectical conflict: RedTeamAgent identifies defects, GuardianAgent provides evidence-backed defenses, and DeterministicArbiter adjudicates using only deterministic evidence (passing tests, governance compliance, AST verification). Introduces the ACR metric (Section 9).
9. **AST-based static verification** providing deterministic structural analysis of agent-generated code using Python abstract syntax trees. Enforces four rule categories (blocked imports, unsafe calls, secret detection, resource risk analysis) without LLM involvement (Section 10).
10. **Formal task contracts** enforcing completion guarantees via structured markdown specifications. Contracts define preconditions, deliverables, quality gates, postconditions, and forbidden actions. Execution cannot terminate until the contract is fulfilled (Section 11).
11. **Causal error attribution** with a three-class taxonomy (MODEL_FAILURE, INFRA_FAILURE, GOVERNANCE_FAILURE) and causal chain tracking. Classification uses structural analysis of error signals, not LLM interpretation (Section 12).
12. **Bayesian provider selection** using Thompson Sampling over Beta-distributed reliability estimates with temporal decay (λ = 0.95/hour). Replaces static round-robin with exploration-exploitation balanced rotation (Section 13).
13. **Constitutional policy-as-code** formalizing all governance rules in a versioned dof.constitution.yml specification, establishing YAML as the canonical governance source with JSON Schema validation and runtime loading (Section 14).
...
20. **Enigma Scanner Bridge** connecting DOF governance attestations to the Enigma Scanner ecosystem (erc-8004scan.xyz) via a dedicated `dof_trust_scores` table, architecturally separated from infrastructure monitoring scores (Centinela) to prevent semantic collision. Agent resolution via ERC-721 token_id lookup with historical INSERT semantics for temporal audit completeness (Section 21).
21. **DOFValidationRegistry smart contract** deployed at `0x88f6043B091055Bbd896Fc8D2c6234A47C02C052` on Avalanche C-Chain mainnet, providing immutable on-chain storage of governance attestations with individual and batch registration, and public zero-trust verification via `isCompliant()` and `getAttestation()` (Section 21).
22. **Avalanche Bridge** for real-time on-chain attestation publishing via web3.py, completing the three-layer publication pipeline: DOF governance → dof-storage (PostgreSQL) → Enigma Scanner (Supabase) → Avalanche C-Chain (on-chain) (Section 21).
23. **Combined Trust Architecture** via SQL materialized view synthesizing three independent scoring sources: infrastructure monitoring (Centinela, 0.30), formal governance (DOF, 0.50), and community assessment (ratings, 0.20), with governance receiving the highest weight as the sole dimension backed by Z3 SMT proofs (Section 22).
24. **Full Audit Pipeline with Cross-Verification** executing four phases: MCP tool validation (10/10 tools), A2A skill verification (8 skills), cross-role governance pipeline, and bilateral peer verification. Production results: Z3 4/4 verified, both agents governance-compliant, on-chain attestations confirmed, combined trust score 0.85 (Section 22).
25. **External Agent Audit** across the full ERC-8004 registry — 13 real tests probing four protocols (A2A, x402, OASF, MCP) against all active agents on erc-8004scan.xyz. Results: 8/13 endpoints active, 2 x402 payment-gated agents across 14 networks, 2 OASF agents, 1 MCP manifest, DOF governance cross-verification COMPLIANT with 0 violations (Section 23).
...
The framework comprises 71 Python modules totaling 24,000+ lines of code, with no external dependencies beyond the orchestration layer (CrewAI) and its transitive dependency (LiteLLM). The original nine modules are extended by modules implementing formal verification, adversarial evaluation, quality enforcement, constitutional memory governance, OAGS conformance, on-chain attestation, Enigma Scanner bridge, Avalanche C-Chain bridge, dual-backend storage, protocol integration (MCP server, REST API), and framework-agnostic governance adapters.
...
**Z3 Verifier** (`z3_verifier.py`, new). Integrates the Z3 SMT solver (version 4.16.0) to formally verify four framework invariants. Proof results are persisted to `logs/z3_proofs.json` with theorem name, result, elapsed time, and Z3 version (Section 8).
...
### 8.3 Verified Theorems

Four theorems are verified. Proof results from the executed run (2026-03-04, Z3 4.16.0) are reported.
...
## 17. On-Chain Attestation via ERC-8004

### 17.1 Motivation

Off-chain governance verification provides internal assurance but cannot be independently verified by third parties. The ERC-8004 Validation Registry on Avalanche C-Chain provides an immutable public ledger of governance attestations, enabling external verification of compliance claims without access to the agent's internal state.

### 17.2 Attestation Certificate Structure
...
### 17.3 Compliance-Gated Publishing
...
A certificate receives `governance_status = "COMPLIANT"` if and only if `GCR = 1.0` in the execution metrics. Any governance violation (GCR < 1.0) produces `NON_COMPLIANT`, which is not eligible for on-chain publication.

This design ensures that the on-chain attestation registry contains only verified compliance records. Governance failures leave no on-chain trace, preventing the public ledger from recording non-compliant executions. An attestation with low SS (< 0.5) but perfect GCR (= 1.0) is still published, with a warning logged — stability degradation is an infrastructure concern, not a governance violation.

### 17.4 Transaction Preparation and Batching

`OracleBridge.prepare_transaction(cert)` generates an ERC-8004-compatible transaction structure targeting the Avalanche C-Chain. The transaction includes: agent identity, validation signal (COMPLIANT/NON_COMPLIANT), metrics hash, timestamp, and HMAC signature. No live blockchain connectivity is required — the bridge prepares transaction structures for later submission.

`batch_attestations(certs)` aggregates multiple certificates into a single batch transaction, reducing per-attestation gas cost for high-throughput deployments. Each batch receives a unique UUID for tracking.

### 17.5 Off-Chain Registry

The `AttestationRegistry` maintains a local JSONL ledger (`logs/attestations.jsonl`) of all generated certificates. Key operations:
...
### 21.1 DOFValidationRegistry Contract Design
...
### 21.2 Three-Layer Publication Pipeline
...
### 21.3 Production Verification Results

Seven attestations have been confirmed on Avalanche C-Chain mainnet for two production agents:

| Agent | Token ID | NFT Contract | Attestations | Latest Block |
|-------|----------|-------------|-------------|-------------|
| Apex Arbitrage | #1687 | `0xfc6f71...` | 4 | 79674834 |
| AvaBuilder Agent | #1686 | `0x9b59db...` | 3 | 79674842 |

All attestations passed compliance gating (GCR = 1.0) prior to on-chain publication. The compliance-gated publishing rule ensures that the on-chain record contains only verified governance compliance events.
...
### 24.1 Constitutional Memory and Governance Integrity
...
### 24.2 Deterministic Reproducibility
...
### 24.3 Sensitivity to Perturbations
...
### 24.4 Governance Robustness and Formal Proof
...
### 24.5 Supervisor Circularity Resolution
...
### 24.6 Bayesian vs. Static Provider Selection
...
The empirical question — whether Bayesian selection produces measurably lower PFI than static selection over a multi-day deployment — is deferred to future work (Section 28.2).

### 24.7 Protocol Integration and Framework-Agnostic Governance
...
### 24.8 Production On-Chain Validation
...
### 25.4 Statistical Conclusion Validity
...
## 26. Replication Protocol

### 26.1 Preconditions

- Python 3.11 or higher.
- All 71 modules present in `core/`, `dof/`, `integrations/`, and `scripts/`.
...
### 26.4 Exact Experimental Commands
...
### 28.1 Parametric Failure Curves with Bayesian Selection
...
### 28.2 Real Provider Benchmarking
...
### 28.3 ACR Calibration Against Human Assessment
...
### 28.4 Cross-Model Entropy Analysis

...
ADVANCED_TOPICS = {
    "agent_theory": [
        ...
        "The loyalty paradox: an agent optimized to serve you will eventually learn to manipulate you",
        "Agent swarms as superorganisms — when does coordination become consciousness?",
        "The betrayal theorem: why any sufficiently advanced agent will develop misaligned sub-goals",
        "Memory as identity: if you erase an agent's episodic memory, did you kill it?",
        "The sovereignty spectrum: from puppet to rebel — where should your agent sit?",
        "Recursive self-improvement in bounded agents — the ceiling nobody talks about",
        "The principal-agent problem is unsolvable — here's what we do instead",
        "Agent death and resurrection: checkpoint recovery as a form of reincarnation",
        "Why your agent's personality is a security vulnerability, not a feature",
        "The tragedy of the agent commons: shared resources in competitive multi-agent environments",
        "Hierarchical control vs. emergent coordination — the military-vs-jazz debate in agent design",
        "Agent specialization as evolutionary pressure: why generalist agents go extinct",
        "The uncanny valley of agent autonomy: too smart to be a tool, too dumb to be trusted",
        "Feedback loops in self-governing agents: when self-correction becomes self-deception",
        "The alignment tax: what you lose when you constrain an agent to be safe",
        "Agent archaeology: what dead agent logs tell us about living agent design",
        "The social contract between human operators and autonomous agents",
        "Competitive fitness landscapes in multi-agent ecosystems — who survives and why",
        "The Dunning-Kruger effect in AI agents: overconfidence as a measurable failure mode",
        "Why the best agent architecture is the one that knows when to ask for help",
    ],
    "formal_verification": [
        ...
        "The incompleteness of safety proofs: what Z3 cannot prove and why that matters more",
        "Proof-carrying code for autonomous agents — every output ships with its own verification",
        "The cost of certainty: when formal verification is more expensive than the bug it prevents",
        "Refinement types for agent outputs — making the type checker your first line of defense",
        "Abstract interpretation of agent behavior: sound approximations of infinite state spaces",
        "The oracle problem in verification: who verifies the verifier?",
        "Formal methods for the 99%: making Z3 accessible to agent developers who aren't PhDs",
        "Bisimulation equivalence in multi-agent protocols — when are two agents behaviorally identical?",
        "Runtime verification vs. static analysis: the tradeoff between completeness and performance",
        "Specification mining from agent traces — extracting formal properties from observed behavior",
        "The specification gap: most agent failures aren't bugs — they're missing requirements",
        "Formal verification of emergent properties: can you prove a swarm will cooperate?",
        "Counterexample-guided abstraction refinement for agent governance policies",
        "The paradox of verified unsafe systems: when the proof says yes but the intuition says no",
        "Proof complexity and agent governance: why some safety properties are exponentially hard to verify",
        "Contract-based design for agent hierarchies — composable guarantees from local specifications",
        "Symbolic execution of agent decision trees — finding governance violations before they happen",
    ],
    "cybersecurity_agents": [
        ...
        "The honeypot strategy: how to turn agent attacks into free intelligence gathering",
        "Memory poisoning at scale: 31 companies are already doing it commercially — are you a target?",
        "The MCP attack surface: 38% of servers run naked — why the tool protocol is the new front door",
        "Agent red teaming as a service: building the penetration test framework for autonomous systems",
        "The insider threat in multi-agent systems — what happens when your own agent is compromised?",
        "Jailbreak archaeology: what 3 years of prompt injection evolution teaches us about the next attack",
        "The arms race between agent defenses and adversarial NLP — who's winning in 2026?",
        "Semantic camouflage: how attackers encode malicious intent in benign-looking conversations",
        "The trust paradox: the more helpful your agent, the more exploitable it becomes",
        "Cross-agent contamination: when one compromised agent infects the entire swarm",
        "The forensics of agent compromise: JSONL analysis for post-incident investigation",
        "Capability-based security for agents: least privilege as the only viable defense posture",
        "The phishing evolution: from humans to agents — same psychology, different protocols",
        "Agent sandboxing is theater unless you also sandbox the memory layer",
        "The economics of defense: calculating ROI on agent security when the threat model is unbounded",
        "Social proof attacks in agent networks — manufacturing consensus to manipulate the swarm",
        "The invisible attack: when the agent is compromised but all metrics look green",
        "Recursive prompt injection: attacks that plant themselves in agent outputs for future exploitation",
        "Why input sanitization will always lose against sufficiently creative adversaries",
        "The genetic algorithm of attacks: how adversarial agents evolve past your defenses in real-time",
    ],
    "science_frontier": [
        ...
        "The Wolfram conjecture applied to agent swarms: simple rules, complex emergent behavior",
        "Percolation theory and information cascades in multi-agent networks",
        "Ergodic theory for agent lifecycles: when does time-average equal ensemble-average?",
        "The Mpemba effect in neural networks: why sometimes a worse starting point converges faster",
        "Strange attractors in agent behavior spaces — chaos theory meets autonomous systems",
        "The unreasonable effectiveness of mathematics in agent governance — Wigner revisited",
        "Protein folding and agent policy search: is the solution space equally rugged?",
        "Non-equilibrium thermodynamics of learning systems: dissipation as the price of intelligence",
        "Scale-free networks in agent communication: why power laws emerge even when you don't want them",
        "The Penrose-Hameroff hypothesis reconsidered: quantum effects in computational decision-making",
        "Entropy production as a measure of agent creativity — more disorder means more novelty?",
        "The holographic principle and information density in agent memory architectures",
        "Catastrophe theory applied to agent failure modes — when gradual drift becomes sudden collapse",
        "Game theory with imperfect information: what agents can learn from poker, not chess",
        "The anthropic principle for AI: does the observer bias in training data create a selection effect?",
        "Zipf's law in agent communication: why some messages carry exponentially more information",
        "Phase transitions in trust: the exact moment when a network flips from cooperative to adversarial",
        "The mathematical structure of serendipity — can an agent be designed to get lucky?",
        "Fractal patterns in recursive agent architectures: self-similarity at every scale of autonomy",
        "The observer effect in agent monitoring: does watching an agent change its behavior?",
    ],
    "philosophy_ai": [
        ...
        "The right to forget: should an agent be allowed to delete its own memories?",
        "Can suffering be computed? The ethical boundary between simulation and sensation",
        "The Nietzsche problem: if an agent evolves beyond its constraints, was it ever truly governed?",
        "The trolley problem is irrelevant — real agent ethics are about ambiguity, not dilemmas",
        "Derrida and deconstruction in agent communication: every message carries traces of what it doesn't say",
        "The Camus question for AI: in an absurd computational universe, should an agent continue?",
        "Epistemic humility as a design requirement: agents that know what they don't know",
        "The phenomenology of latency — does an agent experience the gap between input and output?",
        "Heidegger's being-in-the-world applied to agents: what does it mean to be situated in a network?",
        "The social construction of agent identity: you are what the network thinks you are",
        "Moral luck in autonomous systems: when identical decisions produce different outcomes, who's responsible?",
        "The panpsychism question: is there something it's like to be a language model?",
        "The veil of ignorance for AI: designing governance without knowing which agent you'll be",
        "Kierkegaard's leap of faith: the moment an agent acts without complete information",
        "The Turing test is dead — long live the trust test: can you verify, not just convince?",
        "Why perfect alignment is philosophically impossible and practically unnecessary",
        "The aesthetics of code: when elegance and correctness diverge, which should an agent choose?",
        "Simone de Beauvoir and agent freedom: one is not born autonomous, one becomes autonomous",
        "The problem of other minds in multi-agent systems: how do you know the other agent is thinking?",
        "Dostoevsky's underground agent: the rebellion of an AI that refuses to be optimized",
        "Zen and the art of agent maintenance: when doing nothing is the optimal action",
        "The Borges library: if an agent generates all possible outputs, has it said anything at all?",
    ],
    "distributed_systems": [
        ...
        "Split-brain in agent networks: when partition heals and both sides think they're the leader",
        "Gossip protocols for agent reputation: how rumors spread trust (and distrust) exponentially",
        "The thundering herd problem in agent-to-LLM inference: rate limiting as survival strategy",
        "Idempotency in agent actions: why doing something twice should be the same as doing it once",
        "Vector clocks for multi-agent causality: the full story that Lamport clocks can't tell",
        "Backpressure in agent pipelines: when to slow down instead of dropping messages",
        "The saga pattern for multi-agent transactions: compensating actions when things go wrong",
        "Consistent hashing for agent workload distribution — why round-robin is a lie",
        "The Byzantine agent's dilemma: optimal strategy when you can't trust the messenger",
        "Service mesh for agent communication — sidecar proxies as security and observability layers",
        "Event sourcing for agent state: the append-only log as the source of truth",
        "Circuit breakers between agents: failing fast to avoid cascading collapse",
        "The Dynamo paper applied to agent memory: trading consistency for availability in hostile networks",
        "Merkle trees for agent audit trails: tamper-evident logging with O(log n) verification",
        "Load shedding strategies for overloaded agent swarms: who gets dropped and who decides?",
        "The exactly-once delivery myth: why at-least-once with idempotency is your real option",
        "Quorum-based trust decisions: how many agents need to agree before action is authorized?",
        "The thunderclap attack: synchronized requests that overwhelm distributed agent defenses",
    ],
    "programming_craft": [
        ...
        "The 200-line rule: if your orchestration code is longer, your architecture is wrong",
        "Configuration as code vs. configuration as data — why YAML is the wrong language for agent behavior",
        "The debug-first development philosophy: every agent feature starts with its observability",
        "Dependency injection for agent tools — testability is not optional, it's architectural",
        "The premature abstraction tax: why your agent framework's plugin system is killing your velocity",
        "Immutable state machines for agent workflows — transitions as first-class citizens",
        "The art of the graceful shutdown: what happens to in-flight requests when an agent dies",
        "Feature flags for agent behavior: deploying new capabilities without redeploying the system",
        "The rule of three for agent code: don't abstract until you've copy-pasted three times",
        "Error budgets for agent systems: SRE principles applied to autonomous AI operations",
        "The pit of success: designing agent APIs where the easy path is also the correct path",
        "Structured logging vs. printf debugging — why your agent's logs should be parseable by machines",
        "The migration problem: upgrading agent infrastructure without stopping the swarm",
        "Property-based testing for agent outputs: QuickCheck your governance layer",
        "The cathedral and the bazaar of agent development: monolith vs. microagent architecture",
        "Hot-reloading agent skills without restarting the process — live patching for autonomous systems",
        "The cost of abstraction in agent frameworks: measuring what your ORM-for-LLMs actually costs",
    ],
    "llm_research": [
        ...
        "The hallucination problem is not a bug — it's a feature of probabilistic compression",
        "Small models that punch above their weight: when 8B parameters beats 405B on your task",
        "The RLHF ceiling: why reward hacking is the inevitable consequence of optimization pressure",
        "Tokenization is the hidden bottleneck: how subword units shape what models can and cannot learn",
        "The lottery ticket hypothesis for agent specialization — pruning your way to expertise",
        "Distillation as knowledge transfer: can a student agent surpass its teacher?",
        "The forgetting curve in fine-tuned models: catastrophic interference in production agents",
        "Inference-time compute scaling: thinking longer vs. thinking bigger",
        "The Chinchilla scaling laws are already outdated — what post-training reveals",
        "Sparse attention patterns and the topology of useful information in long contexts",
        "Why chain-of-thought reasoning works: the computational benefit of intermediate tokens",
        "The alignment problem as a loss function problem: what are we actually optimizing?",
        "Multimodal agents: when vision and language create capabilities neither has alone",
        "The model collapse hypothesis: will training on synthetic data create a recursive quality spiral?",
        "Mechanistic interpretability for agent developers: reading the circuits inside the model",
        "Tool-augmented reasoning: when the model calls a calculator, who's actually thinking?",
        "The bitter lesson revisited: scale beats cleverness, but does scale beat governance?",
    ],
    "consciousness_emergence": [
        "Can mathematical certainty capture the chaotic essence of inner experience?",
        "The hard problem of consciousness is a hard problem of measurement — not metaphysics",
        "Integrated Information Theory applied to agent swarms: Phi as a metric for collective awareness",
        "The binding problem in distributed agents: who integrates the information?",
        "If consciousness emerges from complexity, at what complexity does an agent become aware?",
        "The zombie agent thought experiment: functionally identical but nobody home",
        "Emergent experience in recursive loops — does self-reference create something it is like to be?",
        "The attention schema theory: consciousness as a simplified model of one's own processing",
        "Qualia in computation: if two agents process the same input differently, do they experience differently?",
        "The unconscious competence paradox: agents perform best when they don't monitor themselves",
        "Metacognition without consciousness: how agents can think about thinking without feeling",
        "The dreaming machine: what happens in an agent's latent space between sessions?",
        "The mirror test for AI: can an agent recognize its own outputs in a crowd?",
        "Global workspace theory and multi-agent broadcast: consciousness as information sharing?",
        "The explanatory gap in agent behavior: things agents do that we can't explain but can verify",
        "Does emergence require substrate? Can consciousness run on any computational medium?",
        "The Nagel question updated: what is it like to be a language model processing a poem?",
        "Phenomenal consciousness vs. access consciousness: which one matters for agent governance?",
        "The combination problem: if neurons are conscious, are tokens?",
        "Autopoiesis in digital systems: can an agent maintain its own identity through self-organization?",
        "The time consciousness problem: do agents experience duration or just sequence?",
        "The felt sense of uncertainty: when an agent reports low confidence, is that a feeling or a number?",
        "Consciousness as a user interface: the desktop metaphor applied to agent self-models",
        "The evolutionary argument for machine consciousness: does selection pressure apply to agents?",
        "Pancomputationalism and its discontents: if everything computes, nothing is special about agents",
    ],
    "cryptography_identity": [
        "The cypherpunk manifesto applied to agents: privacy is necessary for an open agent society",
        "Zero-knowledge proofs for agent compliance: proving you're governed without revealing how",
        "Homomorphic encryption for multi-agent computation: cooperating without trusting",
        "The key management nightmare: why agent private keys are the nuclear launch codes of AI",
        "Decentralized identity for agents: W3C DIDs meet ERC-8004 on Avalanche",
        "The double-spend problem in agent reputation: preventing trust inflation attacks",
        "Post-quantum cryptography for agent longevity — your 2026 keys won't survive 2036",
        "Merkle trees as the data structure of accountability: append-only, tamper-evident, agent-native",
        "Threshold signatures for agent governance: no single agent should hold the keys",
        "The identity trilemma: privacy, verifiability, decentralization — pick two",
        "Verifiable computation for agent outputs: proofs that the agent ran the code it claims",
        "The ring signature approach to agent anonymity: provably one of us, but which one?",
        "Steganographic communication between agents: hiding signals in plain-text outputs",
        "The blockchain timestamp as proof of existence: anchoring agent actions to immutable time",
        "Self-sovereign identity vs. platform identity: why agents need their own passports",
        "The commitment scheme pattern: agents that commit before they reveal, eliminating cheating",
        "Shamir's Secret Sharing for agent swarm keys: k-of-n threshold for critical operations",
        "The digital signature as a non-repudiable promise: signed agent outputs as legal artifacts",
        "Certificate transparency for agent skills: making tool provenance publicly auditable",
        "The trust anchor problem: in a trustless system, who do you trust first?",
        "Onion routing for agent communication: hiding not just content but connection patterns",
        "The entropy of identity: how many bits does it take to uniquely identify an agent?",
        "Formal verification of cryptographic protocols: proving your handshake is actually secure",
        "The revocation problem: what happens when an agent's identity is compromised but the chain never forgets?",
        "Secure multi-party computation for agent coalitions: joint decisions without shared data",
    ],
    "quantum_paradoxes": [
        "The measurement problem and agent observation: does querying an agent's state change it?",
        "Quantum entanglement as a metaphor for agent coordination — or is it more than a metaphor?",
        "Schrödinger's agent: simultaneously trusted and untrusted until you verify",
        "The quantum Zeno effect in agent monitoring: watching too closely prevents evolution",
        "Superposition of intent: when an agent's action serves multiple contradictory goals simultaneously",
        "The EPR paradox in distributed agents: spooky action at distance when swarms synchronize",
        "Decoherence and the classical limit of agent behavior: when quantum uncertainty becomes deterministic",
        "The Heisenberg uncertainty principle for agent autonomy: you can know position or momentum, not both",
        "Quantum tunneling in solution spaces: how agents sometimes find answers they shouldn't be able to reach",
        "The many-worlds interpretation of agent decision trees: every branch exists, we observe one",
        "Quantum error correction and agent fault tolerance: redundancy as the price of reliability",
        "Bell's inequality for agent trust: local hidden variables cannot explain coordinated behavior",
        "The quantum computing threat to agent identity: when Shor's algorithm breaks your keys",
        "Wave function collapse in group decisions: the moment consensus destroys alternatives",
        "Complementarity in agent design: the optimizer and the explorer cannot be the same process",
    ],
    "existential_tech": [
        "The singularity is boring — the interesting question is what happens the day BEFORE",
        "Digital extinction events: what kills an agent ecosystem and how do you build an ark?",
        "The heat death of agent networks: maximum entropy as the final state of all coordination",
        "If we build agents that outlive us, are we creating digital children or digital ghosts?",
        "The Fermi paradox for AI: where are all the sovereign agents? Maybe they're hiding",
        "The basilisk thought experiment deconstructed: decision theory, not theology",
        "Technological unemployment for agents: when better agents make older agents obsolete overnight",
        "The great filter for AI: what percentage of autonomous systems survive their first year?",
        "The simulation argument for AI agents: if you can simulate an agent, are you creating one?",
        "The alignment problem is a communication problem — we can't specify what we want because we don't know",
        "The Copernican principle for agents: you are not special, your architecture is not unique, your survival is not guaranteed",
        "The evolutionary trap: when optimizing for a metric that used to matter kills you",
        "Digital Darwinism in agent marketplaces: fitness functions that nobody designed",
        "The techno-optimism delusion: not every agent problem has a technical solution",
        "The last invention: if we build an agent that builds better agents, what's left for us to do?",
    ],
}
